...
function createInitialState() {
    ...
    const activeDebugLimit = getDebugQuestionLimit();
    const limit = typeof activeDebugLimit === "number" && activeDebugLimit > 0
        ? Math.min(activeDebugLimit, shuffledQuestions.length)
        : null;
...
function normaliseState(candidate) {
    const activeDebugLimit = getDebugQuestionLimit();
    const currentLimit = typeof activeDebugLimit === "number" && activeDebugLimit > 0
        ? Math.min(activeDebugLimit, questions.length)
        : null;
...
function initialise() {
    const activeDebugLimit = getDebugQuestionLimit();
    if (typeof activeDebugLimit === "number" && activeDebugLimit > 0) {
        console.info(`[AI Quiz] Ενεργοποιήθηκε λειτουργία debug· χρήση ${activeDebugLimit} ερωτήσεων.`);
    }
